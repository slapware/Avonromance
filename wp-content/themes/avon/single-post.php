...
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
	<?php
	$book_vote_count = getvotestypecount($post->ID, "p");
...
//echo "single-post.php<br>";
	$book_title = get_post_meta( get_the_ID(), 'BookTitle', true );
//	var_dump( $book_title);
//	var_dump($book_tag_info);	
	...
	$cover = $covert[0][url];
//	print_r($cover);
//					$author_name = getAuthorName($post->ID);
					...
					<!-- SLAP This nedds to be fixed -->
				
					<?php if($author_name !='' && $author_name !='0'){ ?>
					<li><a href="<?php echo get_permalink($book_author[0]->ID); ?>">By <?php echo $author_name; ?></a></li>
					<?php }  ?>
