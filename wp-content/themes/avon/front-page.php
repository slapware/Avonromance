...
//		$query_posts->query('category_name=Uncategorized&post_type='.$catname.'&orderby=date&order=DESC');
		$query_posts->query('post_type='.$catname.'&orderby=date&order=DESC');
...
//			$book_id = get_post_meta( get_the_ID(), 'BookIsbn', true );
			$book_id = $post->ID;

// NOTE: New slp test Today is 11/8/14. It is now 3:36 PM
			$book_info = get_post($post->ID);
			$book_custom_fields = get_post_custom($post->ID);
//			$book_author = get_field('book_author', $post->ID);
			$book_author = get_post_meta( get_the_ID(), 'BookAuthor', true);
...
//			if($book_custom_fields['cover_image_url'][0] != '')
			if($cover != '')
			{
				$slide_count++;
			}
...
<div class="span8">
	...
	$book_tag_info = getBooksByPostTags($my_query->post->ID);	
//	$book_tag_info = get_post_custom($post->ID);
//	$book_tag_info = get_the_tags();
...
//			var_dump($custom_fields);
 			  ...
 			  $isin = strpos($tag->name, "author:");
 			  if($isin === 0) {
 			  	$author_name = substr($tag->name, 7);
// 			   	echo $tag->name . '<br> '; 
// 			   	log_error($tag->name);
				break;
				}
...
//	$book_info = get_post($book_id);
//	$book_info = get_post($post->ID); // 2/11
	$book_info = get_post($my_query->post->ID);
//	print_r($book_info);
//	$book_custom_fields = get_post_custom($book_id);
	$book_custom_fields = get_post_custom($post->ID);
//	print_r($book_custom_fields);
//	echo $book_info->post_title;
//	$post_vote_count = getvotescount($post->ID);
	$type = "p";
	$post_vote_count = getvotestypecount($post->ID, $type);
//	$book_vote_count = getvotescount($post->ID);
	$type = "b";
	$book_vote_count = getvotestypecount($post->ID, $type);
	...
	<div class="post">
		...
		<div class="span3">
			...
			$custom_fields = get_post_custom($author_obj->ID);
			$author_data = unserialize($custom_fields['badges'][0]);
			$no_of_posts = countPostsByAuthorName($post->ID);
			$covert =  unserialize($book_custom_fields['images'][0]);
			$cover = $covert[0][url];
			// test SLP
			...
			<ul class="info">
			<li><a href="/tidal_contributor/<?php echo $author_obj->post_name; ?>"><?php echo getnamebyslug($author_obj->post_name); ?></a></li>
			<?php if (isset($custom_fields['blog_name']) ) foreach($custom_fields['blog_name'] as $authorblog => $authorbloginfo){ ?>
			...
			<?php if($author_name !='' && $author_name !='0'){ ?>
